...
if __name__ == "__main__":
    # Example: Detect music in a clip
    detector = MusicDetector(api_key="YOUR_API_KEY_HERE")

    # Scan a 60s clip
    detections = detector.scan_clip(
        video_file="path/to/vod.mp4",
        clip_start=120.0,  # Start at 2 minutes
        clip_end=180.0,    # End at 3 minutes
        scan_interval=10   # Scan every 10 seconds
    )

    # Analyze coverage
    analysis = detector.analyze_clip_music_coverage(detections, clip_duration=60.0)

    print("\n📊 Analysis:")
    print(f"   Has music: {analysis['has_music']}")
    print(f"   Coverage: {analysis['coverage_percentage']:.1f}%")
    print(f"   Tracks found: {analysis['unique_tracks']}")
    for track in analysis['tracks']:
        print(f"      - {track}")

    # Should we skip this clip?
    if detector.should_skip_clip(analysis, max_music_percentage=30.0):
        print("   ❌ SKIP - Too much copyrighted music")
    else:
        print("   ✅ OK - Acceptable music level")
